...
def configurar_ambiente():
    #DOCSTRING - O que significa a função
    """
...
def carregar_eventos(caminho_arquivo):
    #entender se a pasta existe e esá tudo configurado
...
def salvar_eventos(caminho_arquivo, eventos):
    try:
        with open(caminho_arquivo, "w", encoding="utf-8") as arquivo:
            json.dump(eventos, arquivo, ident=2, ensure_ascii=False)
        print("Eventos salvos com sucesso!")
    except IOError as e:
        print(f"Erro ao salvar eventos: {e}")

def adicionar_evento(caminho_arquivo, eventos):
    ...
    while not data_hora_valida:
        try:
            data_hora_str = input("Digite a data do evento (DD/MM/AAAA): ")
            datetime_obj = datetime.strptime(data_hora_str, "%d/%m/%Y %H:%M")
            data_hora_valida = True
            data_hora_str = datetime_obj.isoformat()
        except ValueError:
            print("Formato de data e hora inválido. Por favor, tente novamente com o formato (DD/MM/AAAA).")
